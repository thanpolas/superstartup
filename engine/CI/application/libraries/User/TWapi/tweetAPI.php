      ...
      if ($running !== NULL && $running_curl != $running) {
        ...
        if (isset($this->_responses[$key])) {
          ...
          if (isset($response->__resp->code) && $response->__resp->code !== 200 && isset($response->__resp->data)) {
            // 3b Jun/23/11 patch, we get an error for undefined indexes, trying to fix it like that...
            if (isset($response->__resp->data->request) && isset($response->__resp->data->error))
...
class tweetOauth extends tweetConnection {
  ...
  private function _checkLogin() {
    if (isset($_GET['oauth_token'])) {
      ...
      // 3b patched current_url() here cause it returns the URI
      // with a questionmark prepended (?)
  ...
  public function logout() {
    $this->_obj->session->unset_userdata('twitterOauthTokens');
  }
  ...
  private function _getAccessKey() {
    $tokens = $this->_obj->session->userdata('twitterOauthTokens');
  ...
  private function _setAccessKey($access_key) {
    ...
    $this->_obj->session->set_userdata('twitterOauthTokens', $tokens);
  }
