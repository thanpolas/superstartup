...
function Landing(runner) {
  ...
  runner.on('test end', function(test){
    ...
    stream.write(runway());
    stream.write('\n  ');
    stream.write(color('runway', Array(col).join('β')));
    stream.write(plane)
    stream.write(color('runway', Array(width - col).join('β') + '\n'));
    stream.write(runway());
    stream.write('\033[0m');
  });
...
function Progress(runner, options) {
  ...
  options.complete = options.complete || 'β¬';
  options.incomplete = options.incomplete || 'β';
...
function Spec(runner) {
  ...
  runner.on('test', function(test){
    process.stdout.write(indent() + color('pass', '  β¦ ' + test.title + ': '));
  });
